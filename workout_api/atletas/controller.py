...
async def post(db_session: DatabaseDependency, atleta_in: AtletaIn = Body(...)):
    try:
        ...
        centro_treinamento = (
            (await db_session.execute(select(CentroTreinamentoModel).filter_by(nome=atleta_in.centro_treinamento.nome))).scalars().first())
    ...
    except IntegrityError: # Adiciona o tratamento de excessão solicitado.
        raise HTTPException(
...
async def query(db_session: DatabaseDependency, 
                nome: str = Query(None, description='Filtrar atletas por nome'), # Query Parameter "nome", como solicitado
                cpf: str = Query(None, description='Filtrar atletas por CPF'), # Query Parameter "cpf", como solicitado
        ) -> list[AtletaOut]:
...
#Adiciona a Paginação como solicitado
